...
const { Basket_Item, Order, Item, Categoria, User, Item_photo } = require('../models/models')
...
async reg(req, res, next) {
  ...
  async setcatalog(req, res, next) {
    ...
    const file = reader.readFile('SexShop7.xlsx') 
...
async createbasketitem(req, res, next) {
  try {
      ...
      let qauantity = 1
      const device = await Basket_Item.create({photo,name,description,qauantity,price,UserId,ItemId});
